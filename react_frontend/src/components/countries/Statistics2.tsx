...
export const Statistics2 = () => {
    ...
    useEffect(() => {
        setLoading(true);
        const apiUrl = `${BACKEND_API_URL}/country?page=${currentPage}&page_size=${entitiesPerPage}&ordering=-avg_qf`;

        fetch(apiUrl)
            .then((response) => response.json())
            .then((data) => {
                // Sort the editions based on the "country_nr" field
                const sortedCountries = data.results.sort((a: Country, b: Country) => a.edition_nr - b.edition_nr);
    ...
    return (
        <Container>
            <h1>Countries ordered by the nr of editions</h1>
